...
// Mirrors AddMovieSheet's own structure: selection is a Map<Long, MovieSearchResult>
...
class MovieSearchScreenContentTest {
    ...
    private val dune = MovieSearchResult(tmdbId = 438631, title = "Dune", imageUrl = null, year = 2021)
    private val arrival = MovieSearchResult(tmdbId = 329865, title = "Arrival", imageUrl = null, year = 2016)
    private val interstellar = MovieSearchResult(tmdbId = 157336, title = "Interstellar", imageUrl = null, year = 2014)

    private lateinit var stateHolder: MutableState<MovieSearchUiState>
    private var confirmed: List<MovieSearchResult>? = null
    ...
    fun tappingAResult_marksIt_andTappingAgainUnmarksIt() {
        ...
        val row = composeRule.onNodeWithTag(TierDetailTestTags.movieSearchResult(dune.tmdbId))
    ...
    fun selectedCounter_matchesNumberOfMarkedItems() {
        ...
        composeRule.onNodeWithTag(TierDetailTestTags.movieSearchResult(dune.tmdbId)).performClick()
        composeRule.onNodeWithTag(TierDetailTestTags.movieSearchResult(arrival.tmdbId)).performClick()
    ...
    fun marksPersist_throughAnEmptyResultAndAnErrorInBetween() {
        ...
        composeRule.onNodeWithTag(TierDetailTestTags.movieSearchResult(dune.tmdbId)).assertIsSelected()
    }
    ...
    fun confirming_yieldsExactlyTheMarkedSet_inOneCall() {
        setContent(MovieSearchUiState.Success(listOf(dune, arrival, interstellar)))
        composeRule.onNodeWithTag(TierDetailTestTags.movieSearchResult(dune.tmdbId)).performClick()
        composeRule.onNodeWithTag(TierDetailTestTags.movieSearchResult(interstellar.tmdbId)).performClick()
    ...
    fun bottomBar_staysVisible_whenTheResultListDoesNotFitOnScreen() {
        ...
        val manyResults = List(40) { index ->
            MovieSearchResult(tmdbId = index.toLong(), title = "Title $index", imageUrl = null, year = null)
        }
    ...
    fun lastRow_isReachable_andNotCoveredByTheBottomBar() {
        ...
        val lastTag = TierDetailTestTags.movieSearchResult(manyResults.last().tmdbId)
    ...
    fun selectedRowTint_spansTheFullRowWidth() {
        ...
        val rowWidth = composeRule.onNodeWithTag(TierDetailTestTags.movieSearchResult(dune.tmdbId))
